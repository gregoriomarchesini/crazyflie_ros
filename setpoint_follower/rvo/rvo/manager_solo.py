...
class Manager(Node) :
    def __init__(self):
        ...
        for idx, num in enumerate(self.AGENTS_INDICES) : # type: ignore
            callback = lambda x, idx = idx : self.on_agent_message(x, idx)
            self.state_subscribers.append(self.create_subscription(Int32, f"/agent_state{num}", callback, 10))
            self.goals_pub.append(self.create_publisher(Goal, f"/goal{num}", 10))
            # self.get_logger().info(f"{AnsiColor.BOLD_GREEN} created sub/pub for agent {num} with idx {idx} {AnsiColor.RESET}")
        ...
        self.agent_state   = [None for _ in self.drones_names]
...
# ──────── Subscription function ──────────────────────────────────────────────────────────────────
    ...
    def on_agent_message(self,msg, idx):
        # self.get_logger().info(f"{AnsiColor.RED} receveiving state at idx {idx} {AnsiColor.RESET}")
        match (msg.data) :
            case AgentState.TAKEOFF:
                # Logic for ascending to target altitude
                self.agent_state[idx] = AgentState.TAKEOFF # type: ignore

            case AgentState.READY:
                # Logic for mission execution or hovering
                self.agent_state[idx] = AgentState.READY # type: ignore

            case AgentState.LANDING:
                # Logic for descending and disarming
                self.agent_state[idx] = AgentState.LANDING # type: ignore

            case AgentState.STOP:
                self.agent_state[idx] = AgentState.STOP # type: ignore
    ...
    def compute_goals(self) :
        ...
        for idx, i in enumerate(self.AGENTS_INDICES) : # type: ignore
            ...
            if i in known_pos.keys() :
                ...
                point.z = 0.
            goal.pos = point
            self.goals_pub[idx].publish(goal)
...
# ──────── Mainloop ───────────────────────────────────────────────────────────────────────────────
    ...
    def mainloop(self):
        if self.manager_state == ManagerState.WAITING_FOR_ODOMETRY:
            ...
            all_stopped = True
            all_landing = True
            for idx, state in enumerate(self.agent_state) :
                if state != AgentState.READY :
                    all_ready = False
                    not_ready.append(self.drones_names[idx])
                if state != AgentState.STOP :
                    all_stopped = False
                if state != AgentState.LANDING :
                    all_landing = False
            if all_ready:
                self.get_logger().info(f"{AnsiColor.BOLD_GREEN} The RVO is ready. {AnsiColor.RESET}",throttle_duration_sec=5.0)
